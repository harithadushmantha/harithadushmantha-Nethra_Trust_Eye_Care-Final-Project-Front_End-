...
import {  getPatients, getPatientViaOptions } from '../Services/patientsService';
    ...
    const column = [

        {
            path: 'name', lable: 'Name', content: patient =>
                (
                    ...
                    <a href={`/${url}/${patient._id}?id=${patient.nic}&name=${patient.name}&phone=${patient.phone}&district=${patient.districtId.name}&gender=${patient.gender}&register_date=${patient.registeredDate}&dob=${patient.dob}`}>{patient.title}. {patient.name}</a>
                )
        ...
        },
        {
            lable: 'District', content: patient => {
                return patient.districtId.name;
            }
        },
        {
            lable: "Registered Date",
            content: patient => {
                const date = patient.registeredDate.substr(0, 10);
                return date;
            }
        }
            ...
            <div className="col-md-9">
                ...
                {totalCount && stringForprint && startDate && endDate && activePrintButton && startDate < endDate &&
                    <a href={`/printpatients?startdate=${startDate}&enddate=${endDate}&page=${currentPage}&pageSize=${""}`}>
                        <button
                            type="button"  className="btn btn-warning" style={{ marginBottom: "1em",color: "white", backgroundColor: "rgb(238, 112, 9)" }}
                        >
